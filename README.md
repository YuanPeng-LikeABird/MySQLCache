...
Redis is a general Cache System in Backend Applications, it can cache everything(usually not big Data) in memory,
MySQLCache only cache the SELECT result from RDBMS mostly in memory。
...
In most situations,SELECT is same in structure, different in parameters:ex.（1）SELECT score FROM student WHERE name = ‘LiLei’ (2) SELECT score FROM student WHERE name=’Jim’
...
So, how to design SQLTable container? Basically, there is a simple,direct implementation:Map.Key of map is sql string, value is SQLTable.how about this?
...
How solve this problem? There is a solution easy to understand, both efficiently.
...
（2）	Join Table
JoinTable consists of two normal table, assume they are Table A and Table B.
For each record(assure Table A record), update SQLTable A record’s target field values by PK.
（3）	Aggregation Table
Discard the Table.
			 
2.3	How to keep data consistent between Cache and RDBMS? 

The System Architecture of MySQLCache is shown by Picture 4.
  ![System](MySQLCache/picture/4.png)
               Picture 4 System Architecture 
               
Normally, MySQLCache is a standalone middleware, not distributed‌.Because distribute machines increase the complexity to keep Data Consistent and reduce efficiency of cache, so I don’t recommend distributed deployment unless necessary(However, Distributed Cache is supported in the near future, not now).MySQLCache can maintain SQLTables as many as possible, even if out of memory, SQLTables can be compressed, write to Memory-mapped file.And
MySQLCache can be setting to restart once SQLTables is too many so that search become inefficient. Restart is hot, just stop the process, and start new one.(However, restart is not implemented now)
...
For Select Task, Balancer calculate Hash Value(0~n-1, n is quantity of TaskQueues) by SQL string, put the task into TaskQueue indexed by Hash Value. 
It is a good strategy to avoid data racing. SQLTables with same SQLTableSchema are operated in one Worker Thread, so there is no need to lock for safe, and promote multi-thread efficiency.
...
2.4	What is Transaction Isolation Level?
...
In Ubuntu, do command in bash: apt-get install libmysqlcppconn-dev
